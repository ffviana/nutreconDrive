...
def get_allCombinations(ref_parsList, lott_parsList, N_uniqueReps):
  
  all_uniqueLottPars = []
  for p in range(len(ref_parsList)):
    uniqueCombs = get_uniqueCombinations(ref_parsList[p], lott_parsList[p])
    all_uniqueLottPars += uniqueCombs
  
  LotteryCombs = all_uniqueLottPars * N_uniqueReps
  return LotteryCombs

def get_allTrialsDF(st_lotteryCombs, mt_lotteryCombs):
...
def pack_taskParameters(st_refPs, st_lottPs, st_money_refQs, st_money_lottQs, st_cPlus_refQs, st_cPlus_lottQs,
  ...
  st_lotteryCombs = get_allCombinations(st_allRefPars, st_allLottPars, uniqueLott_Nreps)
  mt_lotteryCombs = get_allCombinations(mt_allRefPars, mt_allLottPars, uniqueLott_Nreps)
  allTrials_df = get_allTrialsDF(st_lotteryCombs, mt_lotteryCombs).reset_index(drop=True)

  return allTrials_df

def get_subPars_to_DF(alphas, st_betas, mt_betas, scalingFactors, allTrials_df):
...
def pack_subjectParameters(st_money_alpha, st_cPlus_alpha, st_cMinus_alpha, st_money_beta, st_cPlus_beta, st_cMinus_beta, 
  ...
  subjectTrials_df = get_subPars_to_DF(alphas, st_betas, mt_betas, scalingFactors, allTrials_df.copy())
...
def get_pL_(euL, euR, beta, sFactor):
...
def get_pL(row):
  ...
  pL = get_pL_(euL, euR, beta, sFactor)
  return pL
...
def get_subject_choices(subjectTrials_df):

  ref_cols = column_names[2:4] + [column_names[7]]
  lott_cols = column_names[5:7] + [column_names[8]]

  subjectTrials_df[column_names[11]] = subjectTrials_df.apply(lambda row: get_EU(row, ref_cols), axis=1)
  subjectTrials_df[column_names[12]] = subjectTrials_df.apply(lambda row: get_EU(row, lott_cols), axis=1)
  subjectTrials_df[column_names[13]] = subjectTrials_df.apply(lambda row: get_pL(row), axis=1)
  subjectTrials_df[column_names[14]] = subjectTrials_df.apply(lambda row: get_choice(row), axis=1)
  return subjectTrials_df
...
def get_likelihood(row, params, cols = optimize_cols):
  ...
  ref_EU = get_EU_(row[refP_col], row[refQ_col], ref_alpha)
  # Calculate lottery EU
  lott_EU = get_EU_(row[lottP_col], row[lottQ_col], lott_alpha)
  # Calculate probability of choosing lottery
  pL = get_pL_(lott_EU, ref_EU, beta, sFactor)
...
def get_mt_likelihood(row, params, cols = optimize_cols):
...
# Calculating Begative LogLikelihood

def get_negLogLikelihood(params, args):

  df = args
  # compute likelihood of each choice
  likelihood = df.apply(lambda row: get_likelihood(row, params), axis=1).values
...
def get_st_negLogLikelihood(params, args):
...
def get_mt_negLogLikelihood(params, args):

  df = args
  # compute likelihood of each choice
  likelihood = df.apply(lambda row: get_mt_likelihood(row, params), axis=1).values
  # Take negative of logLikelihood for convention
  negloglikelihood = - np.sum(np.log(likelihood))
  return negloglikelihood
